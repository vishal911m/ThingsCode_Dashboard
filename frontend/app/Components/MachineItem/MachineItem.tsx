...
export default function MachineItem({ machine, onEdit, onDelete }: Props) {
  return (
    <div className="w-[15rem] rounded-lg overflow-hidden shadow hover:shadow-md transition cursor-pointer border border-gray-300 bg-white">
      ...
      <div className="bg-red-600 text-white p-2 flex justify-between items-center">
        <Link href={`/machine/${machine._id}`}>
          <h2 className="text-md font-bold">{machine.machineName}</h2>
        </Link>
        <div className="flex gap-2 text-white text-sm">
          <button title="Edit" className="hover:text-gray-200" onClick={() => onEdit(machine)}>
            ✏️
          </button>
          <button title="Delete" className="hover:text-gray-200" onClick={() => onDelete(machine)}>
            🗑️
          </button>
        </div>
      </div>
