...
import { addLiveJob, fetchJobsByDate, fetchTodayJobs, setSelectedDate } from '@/store/jobsSlice';
...
export default function DashboardPage() {
  ...
  const initialLoadRef = useRef(false); // Prevent double initial load

  const processedMachines = useAppSelector(selectProcessedMachines);
  const selectedDate = useAppSelector((state: RootState) => state.jobs.selectedDate);
  const machines = useAppSelector((state: RootState) => state.machines.machines);
  ...
  useEffect(() => {
    if (!initialLoadRef.current) {
      ...
      // Set today's date first, then fetch jobs through the selectedDate effect
      dispatch(setSelectedDate(new Date()));
    }
  }, [dispatch]);

  // Handle date changes - exactly like context
  useEffect(() => {
    if (selectedDate && initialLoadRef.current) {
      dispatch(fetchJobsByDate(selectedDate));
    }
  }, [dispatch, selectedDate]);

  // WebSocket - exactly like context
  useEffect(() => {
    const isToday = new Date().toDateString() === selectedDate?.toDateString();
    if (!isToday) return; // Only listen to websocket if viewing today
  ...
  // Reset selected date to today on mount - exactly like context
  useEffect(() => {
    dispatch(setSelectedDate(new Date()));
  }, [dispatch]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        {processedMachines.map((machine: any) => (
          <MachineItem
            key={machine._id}
            machine={machine}
            onEdit={(machine) => dispatch(openEditModal(machine))}
